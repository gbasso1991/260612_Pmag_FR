...
from funciones_procesado_2 import medida_cruda, medida_cruda_autom,ajusta_seno, sinusoide,resta_inter,filtrando_ruido,recorte,promediado_ciclos,fourier_señales_5,fourier_señales_6,lector_templog_2,lector_templog,susceptibilidad_M_0
...
if Analisis_de_Fourier==1:
    ...
    ax = fig.add_subplot(1,1,1)
    for i in range(len(fnames_m)):
            plt.plot(Ciclos_eje_H[i]/1000,Ciclos_eje_M_filt[i],'-')
...
#%%%
pendiente = ufloat(np.mean(pendientes), np.std(pendientes))

print(f'Pendiente del paramagneto: {(pendiente):.3e} A/V*s*m\n')
print(f'Pendiente del referencia: {(pendiente_patron_Dy2O3):.3e} A/V*s*m\n')

print(f'Diferencia porcentual: {100*(pendiente.nominal_value-pendiente_patron_Dy2O3.nominal_value)/pendiente_patron_Dy2O3.nominal_value:.2f} %' )

print(f'\nAmplitud promedio de la señal de muestra: {(header_amplitudes):.2uS} mV')
